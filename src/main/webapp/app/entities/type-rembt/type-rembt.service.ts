...
@Injectable()
export class TypeRembtService {
  private resourceUrl = HOST + '/api/type-rembts';
  private resourceSearchUrl = HOST + '/api/_search/type-rembts';

  constructor(private http: Http) {}

  create(typeRembt: TypeRembt): Observable<TypeRembt> {
    const copy = this.convert(typeRembt);
 const options = createRequestOption();
  ...
  update(typeRembt: TypeRembt): Observable<TypeRembt> {
    const copy = this.convert(typeRembt);
 const options = createRequestOption();
  ...
  find(id: number): Observable<TypeRembt> {
  ...
  private convert(typeRembt: TypeRembt): TypeRembt {
    const copy: TypeRembt = Object.assign({}, typeRembt);
    return copy;
  }
}
