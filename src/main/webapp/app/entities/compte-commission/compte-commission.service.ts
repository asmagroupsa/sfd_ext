...
@Injectable()
export class CompteCommissionService {
  private resourceUrl = HOST + '/api/compte-commissions';
  private resourceSearchUrl = HOST + '/api/_search/compte-commissions';

  constructor(private http: Http, private dateUtils: JhiDateUtils) {}

  create(compteCommission: CompteCommission): Observable<CompteCommission> {
    const copy = this.convert(compteCommission);
  ...
  update(compteCommission: CompteCommission): Observable<CompteCommission> {
    const copy = this.convert(compteCommission);
    const options = createRequestOption();
    return this.http
  ...
  find(id: number): Observable<CompteCommission> {
    const options = createRequestOption();
    return this.http
  ...
  private convert(compteCommission: CompteCommission): CompteCommission {
    const copy: CompteCommission = Object.assign({}, compteCommission);
