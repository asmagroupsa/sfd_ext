...
@Component({
  selector: 'jhi-comity-mber',
  templateUrl: './comity-mber.component.html'
})
export class ComityMberComponent implements OnInit, OnDestroy {
  ...
  transition() {
    this.router.navigate(['/entity', 'comity-mber'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: ComityMber) {
    return item.id;
  }
