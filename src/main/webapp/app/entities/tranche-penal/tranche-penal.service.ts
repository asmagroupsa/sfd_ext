...
@Injectable()
export class TranchePenalService {
  private resourceUrl = HOST + '/api/tranche-penals';
  private resourceSearchUrl = HOST + '/api/_search/tranche-penals';

  constructor(private http: Http, private dateUtils: JhiDateUtils) { }

  create(tranchePenal: TranchePenal): Observable<TranchePenal> {
  ...
  update(tranchePenal: TranchePenal): Observable<TranchePenal> {
    const copy = this.convert(tranchePenal);
    const options = createRequestOption();
    return this.http
  ...
  find(id: number): Observable<TranchePenal> {
    const options = createRequestOption();
    return this.http
  ...
  private convert(tranchePenal: TranchePenal): TranchePenal {
    const copy: TranchePenal = Object.assign({}, tranchePenal);
