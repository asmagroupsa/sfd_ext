...
export class PartnerService {
  private resourceUrl = HOST + '/api/partners';
  private resourceSearchUrl = HOST + '/api/_search/partners';
  private resourceUrlBySfd = HOST + '/api/client/liste-partenaire-sfd';
  private resourceByPartnerUrl = HOST + '/api/produits';
  ...
  queryBySfd(req?: any): Observable<ResponseWrapper> {
    const options = createRequestOption(
      Object.assign({}, req, {
        NO_QUERY: true
      })
    ...
    return this.http
      .get(this.resourceUrlBySfd, options).catch((res: Response) => {         if (res.status == 401) EventBus.publish('NOT_AUTHORIZED', true);         return Observable.throw(res);       })
      .map((res: Response) => this.convertResponse(res));
  }

  queryProduitByPartber(req?: any): Observable<ResponseWrapper> {
    const options = createRequestOption(
      Object.assign({}, req, {
        NO_QUERY: true
      })
    ...
    return this.http
      .get(this.resourceByPartnerUrl, options).catch((res: Response) => {         if (res.status == 401) EventBus.publish('NOT_AUTHORIZED', true);         return Observable.throw(res);       })
      .map((res: Response) => this.convertResponse(res));
  }
