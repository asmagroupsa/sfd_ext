...
export class PartnerComponent implements OnInit, OnDestroy {
  ...
  transition() {
    this.router.navigate(['/entity', 'partner'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  private onSuccess(data, headers) {
    ...
    this.partners = data.map(partner => {
      if (partner.logo) this.onLoaded(partner, partner.logo);
      return partner;
    });
    select_init();
  }
  ...
  onLoaded(partner: Partner, url: string = '1.png') {
    ...
    this.http
      ...
      .subscribe(url => {
        partner.picture = this.domSanitizer.bypassSecurityTrustUrl(url);
      });
  }
}
