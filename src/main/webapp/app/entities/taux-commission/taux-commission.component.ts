...
@Component({
  selector: 'jhi-taux-commission',
  templateUrl: './taux-commission.component.html'
})
export class TauxCommissionComponent implements OnInit, OnDestroy {
  currentAccount: any;
  tauxCommissions: TauxCommission[];
  error: any;
  success: any;
 ...
 //this.page = page;
  ...
  transition() {
    this.router.navigate(['/taux-commission'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: TauxCommission) {
    return item.id;
  }
