...
@Injectable()
export class TauxCommissionService {
  private resourceUrl = HOST + '/api/taux-commissions';
  private resourceSearchUrl = HOST + '/api/_search/taux-commissions';

  constructor(private http: Http, private dateUtils: JhiDateUtils) {}

  create(tauxCommission: TauxCommission): Observable<TauxCommission> {
    const copy = this.convert(tauxCommission);
  ...
  update(tauxCommission: TauxCommission): Observable<TauxCommission> {
    const copy = this.convert(tauxCommission);
    const options = createRequestOption();
    return this.http
  ...
  find(id: number): Observable<TauxCommission> {
    const options = createRequestOption();
    return this.http
  ...
  private convert(tauxCommission: TauxCommission): TauxCommission {
    const copy: TauxCommission = Object.assign({}, tauxCommission);
