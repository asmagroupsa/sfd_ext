...
@Injectable()
export class SettingSFDService {
  private resourceUrl = HOST + '/api/setting-sfds';
  private resourceSearchUrl = HOST + '/api/_search/setting-sfds';

  constructor(private http: Http) {}

  create(settingSFD: SettingSFD): Observable<SettingSFD> {
    const copy = this.convert(settingSFD);
 const options = createRequestOption();
  ...
  update(settingSFD: SettingSFD): Observable<SettingSFD> {
    const copy = this.convert(settingSFD);
 const options = createRequestOption();
  ...
  find(id: number): Observable<SettingSFD> {
  ...
  private convert(settingSFD: SettingSFD): SettingSFD {
    const copy: SettingSFD = Object.assign({}, settingSFD);
    return copy;
  }
}
