...
import { SFD } from '../s-fd/sfd.model';
...
export class ContratComponent implements OnInit, OnDestroy {
  ...
  sfd(id) {
    if (!this.sfds) return new SFD();
    return this.sfds.find((sfd: SFD) => {
      return sfd.id == id;
    });
  }
  getSFDs() {
    this.sfdService.query().subscribe((res: ResponseWrapper) => {
      this.sfds = res.json;
    });
  }
  produit(id) {
    if (!this.produits) return new SFD();
    return this.produits.find((produit: Produit) => {
 ...
 //this.page = page;
  ...
  printContrat(contrat: any) {
    alert('contrat printed');
  }
}
