import { Injectable, Component } from '@angular/core';
import { Router } from '@angular/router';
import { NgbModal, NgbModalRef } from '@ng-bootstrap/ng-bootstrap';
import { CompteComptable } from './compte-comptable.model';
import { CompteComptableService } from './compte-comptable.service';

@Injectable()
export class PeriodicityPopupService {
  private isOpen = false;
  constructor(
    private modalService: NgbModal,
    private router: Router,
    private periodicityService: CompteComptableService
  ) {}

  open(component: Component, id?: number | any): NgbModalRef {
    if (this.isOpen) {
      return;
    }
    this.isOpen = true;

    if (id) {
      this.periodicityService.find(id).subscribe(periodicity => {
        this.periodicityModalRef(component, periodicity);
      });
    } else {
      return this.periodicityModalRef(component, new CompteComptable());
    }
  }

  periodicityModalRef(
    component: Component,
    periodicity: CompteComptable
  ): NgbModalRef {
    const modalRef = this.modalService.open(component, { keyboard:false,
      size: 'lg',
      backdrop: 'static'
    });
    modalRef.componentInstance.compteComptable = periodicity;
    modalRef.result.then(
      result => {
        this.router.navigate(
          ['/entity', 'compte-comptable', { outlets: { popup: null } }],
          { replaceUrl: true }
        );
        this.isOpen = false;
      },
      reason => {
        this.router.navigate(
          ['/entity', 'compte-comptable', { outlets: { popup: null } }],
          { replaceUrl: true }
        );
        this.isOpen = false;
      }
    );
    return modalRef;
  }
}
