...
export class CommissionService {
  private resourceUrl = HOST + '/api/commissions';
  private resourceSearchUrl = HOST + '/api/_search/commissions';

  constructor(private http: Http, private dateUtils: JhiDateUtils) {}

  create(commission: Commission): Observable<Commission> {
    const copy = this.convert(commission);
 const options = createRequestOption();
  ...
  update(commission: Commission): Observable<Commission> {
    const copy = this.convert(commission);
 const options = createRequestOption();
  ...
  find(id: number): Observable<Commission> {
  ...
  private convert(commission: Commission): Commission {
    const copy: Commission = Object.assign({}, commission);
