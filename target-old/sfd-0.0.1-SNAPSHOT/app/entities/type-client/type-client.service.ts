...
export class TypeClientService {
  private resourceUrl = HOST + '/api/type-clients';
  private resourceSearchUrl = HOST + '/api/_search/type-clients';
  ...
  update(typeClient: TypeClient): Observable<TypeClient> {
    const copy = this.convert(typeClient);
    const options = createRequestOption();
    return this.http
  ...
  find(id: number): Observable<TypeClient> {
    const options = createRequestOption();
    return this.http
  ...
  query(req?: any): Observable<ResponseWrapper> {
    const options = createRequestOption(
      Object.assign({}, {
        NO_QUERY: true,
        'code.in': 'MUTUEL,INDIVIDU,ENTREPRISE,MARCHAND'
      }, req)
    );
    return this.http
