...
export class EcritureService {
  private resourceUrl = HOST + '/api/ecritures';
  private resourceSearchUrl = HOST + '/api/_search/ecritures';

  constructor(private http: Http) { }

  create(ecriture: Ecriture): Observable<Ecriture> {
    const copy = this.convert(ecriture);
  ...
  update(ecriture: Ecriture): Observable<Ecriture> {
    const copy = this.convert(ecriture);
    const options = createRequestOption();
  ...
  find(id: number): Observable<Ecriture> {
  ...
  private convert(ecriture: Ecriture): Ecriture {
    const copy: Ecriture = Object.assign({}, ecriture);
    return copy;
  }
}
